...
class Partitioning():
    ...
    def create_partitioning(self, collist, table, min_year, max_year, cur):
        ...
        self.logger.info(f"Creating partition for {table['schema']}.{table['name']}")
        self.logger.debug("Altering table name")
        cur.execute(alter_tablename)
        self.logger.debug("Create partitioning")
        cur.execute(partitioning)
        self.logger.debug("Alter check constraint for partition table")
        cur.execute(alter_check_constraint)
        self.logger.debug("Attach table to partition table")
        cur.execute(alter_table_attach_partition)
        self.logger.debug("Create a new partition")
        cur.execute(create_new_year_partition)
        self.logger.debug("Change new partition ownership")
        cur.execute(change_new_year_owner)
        self.logger.debug("Add constraint to new partition")
        cur.execute(add_constraint_new_year_partition)
        self.logger.debug("Create default partition to store unpartition stuff")
        cur.execute(create_default_partition)
        self.logger.debug("Change default partition ownership")
        cur.execute(change_default_owner)

    def split_partition(self, table, year, min_year, cur, colname):
        ...
        self.logger.info(f"Splitting partition by year: {year}")
        self.logger.debug("Detach old partition table")
        cur.execute(detach_old_partition)
        self.logger.debug("Create new table for partition")
        cur.execute(create_table)
        self.logger.debug("Change table partition ownership")
    ...
    def split_default_partition(self, table, year, cur, colname):
        ...
        self.logger.info(f"Splitting partition by year: {year}")
        self.logger.debug("Detach default partition table")
    ...
    def checker_table(self, table, cur):
        ...
        self.logger.info(f"Checking table if it is partition: {table['schema']}.{table['name']}")
        cur.execute(checker)
        data = cur.fetchall()
    ...
    def additional_partitioning(self, table_data, cur, table, years, colname):

        data = self.checker_table(table_data, cur)

        if len(data) == 0:
            self.split_default_partition(table, years, cur, colname)
        else:
            self.logger.info(f"Partition already exist for table: {table['schema']}.{table['name']}")

    def check_addon_partition_needed(self, cur, table):
            ...
            with open('config.yaml','r') as yamlfile:
                ...
                for table in cur_yaml['table']:
                    if table['name'] in get_tb_schema[1] and table['schema'] in get_tb_schema[0]:
                        self.logger.info("Adding additional index for reverse partitioning")
    ...
    def addon_partition(self, cur, table, colname):
        ...
        for date in dates:
            ...
            for year in reversed(range(1, update_loop_year + 1)):
                ...
                self.additional_partitioning(new_table, cur, table, years, colname)
            ...
            self.additional_partitioning(min_table, cur, table, min_check_year, colname)

    def change_table_index(self, table, cur):
    ...
    def perform_partition(self, server, table):
        ...
        try:
            ...
            if partitioning:
                ...
                self.create_partitioning(collist, table, min_year, max_year, cur)

                create_loop_year = max_year - min_year

                for year in reversed(range(1, create_loop_year + 1)):
                    new_year = min_year + year 
                    self.split_partition(table, new_year, min_year, cur, colname)
                ...
                for indexes in index_of_old_data:
                    self.logger.info("Dropping index from the old partition data")
                    
                    drop_existing_constraint = drop_table_constraint.format(a=old_tb_name, b=indexes[0])
                    cur.execute(drop_existing_constraint)
                    self.logger.debug(f"Dropped contraint from the {old_tb_name}")

                    drop_index = drop_table_index.format(a=indexes[0])
                    cur.execute(drop_index)
                    self.logger.debug(f"Dropped index from the {old_tb_name}")

                for index in index_data:
                    self.logger.info("Adding index to the new partition table")
                    cur.execute(f"{index};")
                    
                    self.additional_index_reverse_partitioning(index, table)
                ...
                default_table_count = self.check_addon_partition_needed(cur, table)

                if default_table_count[0][0] > 0:
                    ## Create partition for the table and move the data there (yearly basis)
                    self.addon_partition(cur, table, colname)
            conn.commit()
    ...
    def main(self):
        ...
        for table in config['table']:
            ...
            self.perform_partition(server, table)
            toc = time.perf_counter()
            self.logger.debug(f"Script completed in {toc - tic:0.4f} seconds")
