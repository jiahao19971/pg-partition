...
def create_partitioning(collist, table, min_year, max_year, cur):
    ...
    """
    another_new_year = new_year + 1

    create_another_new_year_partition = f"""
        CREATE TABLE {table['name']}_{another_new_year} PARTITION OF {table['name']}
        FOR VALUES FROM ('{another_new_year}-01-01 00:00:00') TO ('{another_new_year + 1}-01-01 00:00:00');
    """

    add_constraint_another_new_year_partition = f"""
        ALTER TABLE {table['name']}_{another_new_year} ADD CONSTRAINT {table['name']}_{another_new_year}
        CHECK  (created >= '{new_year}-01-01 00:00:00' AND created < '{another_new_year + 1}-01-01 00:00:00');
    """
    ...
    print("Create another partition for emergency")
    cur.execute(create_another_new_year_partition)
    print("Add constraint to another new partition")
    cur.execute(add_constraint_another_new_year_partition)
