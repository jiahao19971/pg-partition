...
def pg_create_table(table, year):
    server = _get_tunnel()

    application_name = f"PG_Create_Partition_Table"
    logger = logs(application_name=application_name)
...
def pg_move_table(table, year):
    server = _get_tunnel()
    _, colname = get_column(table)

    application_name = f"PG_Move_Table_To_Partition"
    logger = logs(application_name=application_name)
    conn = DBLoader(server, os.environ['DATABASE'], application_name=application_name)
    conn = conn.connect()
    cur = conn.cursor()
...
def split_partition(table, min_year, years):
    server = _get_tunnel()
    application_name = f"{table['schema']}.{table['name']}"
    logger = logs(application_name=application_name)
    conn = DBLoader(server, os.environ['DATABASE'], application_name=application_name)
    ...
    logger.debug("Create new partition table with multiprocessing")

    create_tables = []
    for year in years:
        create_table = Process(target=pg_create_table, args=(table, year))
        create_table.daemon = True
    ...
    move_tables = []
    for year in years:
        move_table = Process(target=pg_move_table, args=(table, year))
        move_table.daemon = True
        move_tables.append(move_table)
        move_table.start()

    for move_table in move_tables:
        move_table.join()
    ...
    change_constraint = alter_table_constraint.format(a=table['name'], b='old', c=table['partition'], d=min_year, e=min_year + 1)

    reattach_table = attach_table_partition.format(a=table['name'], b=min_year, c=min_year + 1)
...
def get_min_max_date(table, cur):
...
def drop_idx(indexes, old_tb_name):
    try:
        server = _get_tunnel()
        application_name = f"Dropped_Old_Table_Index"
        logger = logs(application_name=application_name)
        conn = DBLoader(server, os.environ['DATABASE'], application_name=application_name)
        conn = conn.connect()
        cur = conn.cursor()

        logger.info(f"Dropping index from the old partition data: {indexes}")
        ...
        logger.error(e)
        logger.error("Error occured while dropping index, rolling back")
        conn.rollback()
    finally:
        conn.close()

def get_drop_old_table_index(table, old_tb_name):
    server = _get_tunnel()
    application_name = f"{table['schema']}.{table['name']}"
    conn = DBLoader(server, os.environ['DATABASE'], application_name=application_name)
    conn = conn.connect()
    ...
    for indexes in index_of_old_data:
        idx = Process(target=drop_idx, args=(indexes, old_tb_name))
        idx.daemon = True
...
def add_new_index_if_any(index, table):
    server = _get_tunnel()
    application_name = f"{table['schema']}.{table['name']}"
    conn = DBLoader(server, os.environ['DATABASE'], application_name=application_name)
    conn = conn.connect()
...
def perform_partitioning(table):
    ...
    try:
        ...
        if partitioning:
            min_year, max_year = get_min_max_date(table, cur)
            ...
            split_partition(table, min_year, years)
            ...
            get_drop_old_table_index(table, old_tb_name)

            if len(index_data) > 0:
                idx_workers = []
                for index in index_data:
                    idx_worker = Process(target=add_new_index_if_any, args=(index, table))
                    idx_worker.daemon = True
                    idx_workers.append(idx_worker)

                for idx in idx_workers:
                    idx.start()
